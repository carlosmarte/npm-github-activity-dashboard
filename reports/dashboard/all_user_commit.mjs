...
class AllUserCommit {
    ...
    if (dateRange && !this.config.ignoreDateRange) {
      ...
      for (const partition of partitions) {
        ...
        const partitionResults =
          await this.searchPullRequestsForQuery(partitionQuery);
  ...
  async analyzeUserCommits() {
    ...
    if (!this.config.ignoreDateRange && this.config.start && this.config.end) {
      console.log(
        `📅 Date range        ${chalk.green(this.config.start)} to ${chalk.green(this.config.end)}`
      );
    } else {
    ...
    console.log(
      `🏢 Organization      ${chalk.green(this.config.org || "All organizations")}`
    );
    console.log(
      `📂 Repositories      ${chalk.green(this.config.repo || "All repositories")}`
    );
    console.log(
    ...
    console.log(
      `📊 Include details   ${this.config.includeDetails ? chalk.green("Enabled") : chalk.gray("Disabled")}`
    );
    console.log(
      `🔍 Verbose mode      ${this.config.verbose ? chalk.green("Enabled") : chalk.gray("Disabled")}`
    );
    console.log(
      `🐛 Debug mode        ${this.config.debug ? chalk.green("Enabled") : chalk.gray("Disabled")}`
    );
    console.log(
      `📊 Total records limit ${this.config.totalRecords > 0 ? chalk.green(this.config.totalRecords) : chalk.yellow("No limit")}`
    );
    console.log("");
    ...
    console.log(
      `📝 Direct commits         : ${chalk.green(this.commits.filter((c) => c.type === "direct").length)}`
    );
    console.log(
      `🔀 Pull request commits   : ${chalk.green(this.commits.filter((c) => c.type === "pull_request").length)}`
    );
    console.log(
      `📂 Unique repositories    : ${chalk.green(new Set(this.commits.map((c) => c.repository)).size)}`
    );
    console.log(`❌ Errors encountered     : ${chalk.red(this.errors.length)}`);
    console.log(
      `📊 Records fetched/limit  : ${chalk.green(this.totalFetched)}${this.config.totalRecords > 0 ? `/${this.config.totalRecords}` : ""}`
    );
    console.log("─".repeat(50));
    console.log("");
  }
  ...
  async generateReport() {
    ...
    const report = {
      ...
      criteria: [
        ...
        `Total records limit enforced: ${this.config.totalRecords > 0 ? this.config.totalRecords : "No limit"}`,
      ],
      formula: [
    ...
    if (this.config.format === "json") {
      ...
      const csvRows = this.commits
        .map(
          (commit) =>
            `"${commit.sha}","${commit.message.replace(/"/g, '""')}","${commit.date}","${commit.repository}","${commit.type}","${commit.pullRequest || ""}","${commit.author}","${commit.url}","${commit.stats?.total || 0}","${commit.stats?.additions || 0}","${commit.stats?.deletions || 0}","${commit.files?.length || 0}","${commit.parents?.length || 0}"`
        )
        .join("\n");
  ...
  async run() {
    try {
      ...
      console.log(
        `📊 Date Range     : ${chalk.green(this.config.start || "N/A")}/${chalk.green(this.config.end || "N/A")}`
      );
      console.log(
      ...
      console.log(
        `📂 Repositories Analyzed  : ${chalk.green(new Set(this.commits.map((c) => c.repository)).size)}`
      );
      ...
      console.log(
        `📊 Records Fetched        : ${chalk.green(this.totalFetched)}${this.config.totalRecords > 0 ? `/${this.config.totalRecords}` : ""}`
      );
      console.log(
        `📄 Report Location        : ${chalk.green(path.join(this.config.outputDir, `${this.config.filename || this.config.searchUser + "-commits"}.${this.config.format}`))}`
      );
      console.log("─".repeat(60));
    } catch (error) {
